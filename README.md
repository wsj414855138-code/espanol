...
**核心思路**：你（老公）负责"喂料"——把培训班教材的内容整理成简单的 Markdown 文件；系统负责"烹饪"——自动生成结构化学习包、西语音频、练习页。她打开网页就能练：词汇卡、听辨、跟读、听写、📊 报告（打卡热力图/连续天数/到期复习卡）。学习记录自动云端同步（Supabase，离线可用、换设备不丢）。
...
| 角色 | 做什么 |
|---|---|
| **老婆（学习者）** | 打开练习页 → 选课程 → 练词汇卡 / 听辨 / 跟读 / 听写；「📊 报告」看打卡热力图与复习状态。**先读 [docs/learner-guide.md](docs/learner-guide.md)：每天 15 分钟"基础巩固循环"，四步固定流程，强调出声练习** |
